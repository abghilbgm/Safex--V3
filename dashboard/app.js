...
async function loadCameras() {
  try { const res = await fetch(API.cameras); state.cameras = await res.json(); }
  catch (e) { state.cameras = []; }
  renderFeedsGroupedByArea(); renderCamerasTable(); populateViolationCameraFilter();
...
function renderFeedsGroupedByArea() {
  const container = document.getElementById("feedsByGroup"); container.innerHTML = "";
  const groups = {};
  state.cameras.forEach((cam) => {
  ...
  groupNames.forEach((groupName) => {
    ...
    section.innerHTML = `<div class="feed-group-header"><h4>${groupName}</h4><span class="feed-group-count">${cams.length} camera${cams.length !== 1 ? "s" : ""}</span></div><div class="camera-grid" id="grid-${groupName.replace(/\s+/g, "_")}"></div>`;
    container.appendChild(section);
    const grid = section.querySelector(".camera-grid");
    cams.forEach((cam) => {
      const tile = document.createElement("div"); tile.className = "camera-tile"; tile.id = `tile-${cam.camera_id}`;
      tile.innerHTML = `<div class="feed-wrap"><canvas id="canvas-${cam.camera_id}"></canvas><div class="feed-placeholder" id="placeholder-${cam.camera_id}">Connecting…</div><div class="violation-flash" id="flash-${cam.camera_id}"></div></div><div class="tile-footer"><div><span class="tile-cam-name">${cam.name}</span><span class="tile-cam-zone">${cam.camera_id}</span></div><div class="tile-status ${cam.connected ? "online" : "offline"}" id="status-${cam.camera_id}"><span class="dot"></span><span>${cam.connected ? "ONLINE" : "OFFLINE"}</span></div></div>`;
...
function connectVideoStream(cameraId) {
  if (state.videoSockets[cameraId]) { try { state.videoSockets[cameraId].close(); } catch (e) {} }
  const proto = location.protocol === "https:" ? "wss" : "ws";
  const ws = new WebSocket(`${proto}://${location.host}/ws/video/${cameraId}`);
  ws.binaryType = "arraybuffer"; state.videoSockets[cameraId] = ws;
  const canvas = document.getElementById(`canvas-${cameraId}`); if (!canvas) return;
  const placeholder = document.getElementById(`placeholder-${cameraId}`); const ctx = canvas.getContext("2d");
  ...
  ws.onclose = () => {
    if (document.getElementById(`placeholder-${cameraId}`)) {
      const p = document.getElementById(`placeholder-${cameraId}`); p.style.display = "block"; p.textContent = "Reconnecting…";
    }
...
async function refreshCameraStatuses() {
  try {
    const res = await fetch(API.cameras); const cams = await res.json(); state.cameras = cams;
    cams.forEach((cam) => {
      const statusEl = document.getElementById(`status-${cam.camera_id}`); if (!statusEl) return;
...
async function refreshCameraConnection(cameraId) {
  ...
  try {
    ...
    connectVideoStream(cameraId); await loadCameras();
...
async function loadViolationsGallery() {
  ...
  let rows = [];
  try { const res = await fetch(`${API.violations}?${params.toString()}`); rows = await res.json(); }
  catch (e) { gallery.innerHTML = `<div class="event-empty">Failed to load violations.</div>`; return; }
  if (!rows.length) { gallery.innerHTML = `<div class="event-empty">No violations found for this filter.</div>`; return; }
  gallery.innerHTML = "";
  rows.forEach((row) => {
    ...
    card.innerHTML = `<div class="thumb-wrap">${snapshotUrl ? `<img src="${snapshotUrl}" alt="${label}" loading="lazy" />` : `<div class="thumb-placeholder">No image</div>`}<span class="thumb-badge">${label.toUpperCase()}</span></div><div class="card-meta"><div class="card-cam">${row.camera_name || row.camera_id}</div><div class="card-time">${row.area_group || row.zone || ""} · ${detectedAt}</div></div>`;
...
async function refreshStats() {
  try {
    const res = await fetch(`${API.stats}?window_hours=24`); const stats = await res.json();
...
async function refreshRules() {
  try { const res = await fetch(API.rules); state.rules = await res.json(); }
  catch (e) { state.rules = []; }
